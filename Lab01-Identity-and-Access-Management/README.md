...
## Scenario
- Tremols Tech is preparing an Azure environment for upcoming engineering work. Before any virtual machines or services can be deployed, the identity layer must be established. Engineers need accounts, groups must be defined, and access must be structured in a way that scales. To reduce administrative overhead, group membership should update automatically based on user attributes such as job title. This is where dynamic groups come into play.
...
- Built a security group for Tremols AZ Administrators to centralize access.
...
## Key Observations
- Tenant switching in Entra ID is smoother than expected, which is very useful for MSP scenarios where multiple tenants are common.
...
- The Properties tab for users contains the metadata fields which are helpful for automation, governance, and conditional access policies.
